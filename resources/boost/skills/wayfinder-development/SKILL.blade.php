...
description: "Use this skill for Ugarit Wayfinder which auto-generates typed functions for Ugarit controllers and routes. ALWAYS use this skill when frontend code needs to call backend routes or controller actions. Trigger when: connecting any React/Vue/Svelte/Inertia frontend to Ugarit controllers, routes, building end-to-end features with both frontend and backend, wiring up forms or links to backend endpoints, fixing route-related TypeScript errors, importing from @/actions or @/routes, or running wayfinder:generate. Use Wayfinder route functions instead of hardcoded URLs. Covers: wayfinder() vite plugin, .url()/.get()/.post()/.form(), query params, route model binding, tree-shaking. Do not use for backend-only tasks"
license: MIT
metadata:
  author: ugarit
---
...
```bash
{{ $assist->scribeCommand('wayfinder:generate --no-interaction') }}
```
For form helpers, use `--with-form` flag:
```bash
{{ $assist->scribeCommand('wayfinder:generate --with-form --no-interaction') }}
```
### Import Patterns
...
@if($assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_UGARIT) || $assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_REACT) || $assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_VUE) || $assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_SVELTE))
## Wayfinder + Inertia
...
@endboostsnippet
@endif
@if($assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_VUE))
@boostsnippet("Wayfinder Form (Vue)", "vue")
<Form v-bind="store.form()"><input name="title" /></Form>
@endboostsnippet
@endif
@if($assist->hasPackage(\Ugarit\Boost\Support\PackageRegistry::INERTIA_SVELTE))
...
1. Run `{{ $assist->scribeCommand('wayfinder:generate') }}` to regenerate routes if Vite plugin isn't installed
