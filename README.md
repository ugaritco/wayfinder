<p align="center"><img src="./art/logo.svg" width="50%" alt="Ugarit Wayfinder Logo"></p>

## Introduction

Ugarit Wayfinder bridges your Ugarit backend and TypeScript frontend with zero friction. It automatically generates fully-typed, importable TypeScript functions for your controllers and routes — so you can call your Ugarit endpoints directly in your client code just like any other function. No more hardcoding URLs, guessing route parameters, or syncing backend changes manually.
...
> [!NOTE]
> Want to try the next version of Wayfinder? [You can find the beta here](https://github.com/ugarit/wayfinder/tree/next).
...
```
composer require ugarit/wayfinder
```

Next, install the [Wayfinder Vite plugin](https://github.com/ugarit/vite-plugin-wayfinder) to ensure that your routes are generated during Vite's build step and also whenever your files change while running the Vite's dev server.

First, install the plugin via NPM:

```
npm i -D @ugarit/vite-plugin-wayfinder
```
...
You can read about all of the plugin's configuration options in the [documentation](https://github.com/ugarit/vite-plugin-wayfinder).
...
```
php scribe wayfinder:generate --path=resources/js/wayfinder
```
...
```
php scribe wayfinder:generate --skip-actions
php scribe wayfinder:generate --skip-routes
```
...
Wayfinder reads routes from the registered router, so if Ugarit boots with a cached route table left over from a previous release, generation will run against those stale routes. Any routes added since the last `route:cache` will be silently missing from the generated `routes/` and `actions/` directories, and Vite will fail with errors like `Could not load resources/js/routes/<name>`.

If your deploy script runs `php scribe optimize` (or `route:cache` directly) at the end of a deploy, run `php scribe route:clear` before regenerating on the next deploy. With the Vite plugin, this needs to happen before `npm run build`, since the plugin invokes `wayfinder:generate` during the build:

```shell
php scribe route:clear
npm run build
```
...
```shell
php scribe wayfinder:generate --with-form
```
...
Please review [our security policy](https://github.com/ugarit/wayfinder/security/policy) on how to report security vulnerabilities.
